...
///
/// Everything still held is written back on drop, so a panic mid-batch — from an oversized
/// key or value, say — cannot leave buffered entries unwritten or `length` out of step
/// with the tree.
struct BulkInsert<'a, K, V, M>
where
    K: Storable + Ord + Clone,
    V: Storable,
    M: Memory,
...
where
    ...
    M: Memory,
{
    fn new(map: &'a mut BTreeMap<K, V, M>) -> Self {
        Self {
            map,
            path: Vec::new(),
            header_dirty: false,
    ...
    fn insert(&mut self, key: K, value: V) {
        // Serialize first: this is the step that can panic, and doing it before any node is
        // touched keeps the panic clean.
    ...
    M: Memory,
{
    ...
    /// path in memory and writes a node out only once the input has moved past it. It also
    /// writes the header once for the whole call rather than once per key.
    ///
    /// The iterator is consumed lazily and never collected, so the memory cost is one tree
    /// path regardless of how many pairs are supplied.
    ///
    /// # Supply keys in sorted order
    ///
    /// The coalescing pays off exactly when consecutive keys land in the same node, so
    /// **feed the pairs in sorted key order**. Ascending is the natural choice, but
    /// descending works just as well — what matters is that consecutive keys are close
    /// together, not which way the run travels.
    ///
    /// Out-of-order keys are handled correctly — the path unwinds to an ancestor that
    ...
    /// and the batch is worth sorting.
    ///
    /// Duplicate keys are applied in the order given, so the last occurrence wins, exactly
    /// as with repeated `insert` calls. The resulting map is identical to what those calls
    /// would produce; only the number of writes differs.
    ///
    /// # Panics
    ///
    /// Panics if a serialized key or value exceeds the maximum size of its type, exactly as
    /// [`insert`](Self::insert) does. Pairs consumed before the panic are already in the
    /// map, and the map is left consistent.
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    pub fn insert_many(&mut self, entries: impl IntoIterator<Item = (K, V)>) {
        // `BulkInsert` writes back everything it is holding when it is dropped, so the map
        // stays consistent even if `into_bytes_checked` panics part way through.
